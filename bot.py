          ...
          if FORCE_SUB_CHANNEL5:
           ...
           if FORCE_SUB_CHANNEL6:
            try:
                ...
                self.LOGGER(__name__).warning(a)
                self.LOGGER(__name__).warning("Bot can't Export Invite link from Force Sub Channel!")
                self.LOGGER(__name__).warning(f"Please Double check the FORCE_SUB_CHANNEL value and Make sure Bot is Admin in channel with Invite Users via Link Permission, Current Force Sub Channel Value: {FORCE_SUB_CHANNEL6}")
                self.LOGGER(__name__).info("\nBot Stopped. https://t.me/MadflixBots_Support for support")
                sys.exit()     
        try:
